...
其中一些证明相当于对经典证明的直接转写；
另一些则更具类型论风格，主要由
$\infty$-群胚运算的计算构成，其风格与计算机科学家使用类型论推理程序非常相似。
...
理解这种复杂性的一种方式，是通过
\cref{cha:basics} 引入的空间与 $\infty$-群胚之间的对应。
\index{.infinity-groupoid@$\infty$-groupoid}%
...
从而确定一个等价，再把该等价作用于 $0$。
这个函数的有趣之处在于：当圆上的回路以同伦类型论抽象的群胚框架表示时，它能从中计算出一个具体整数。
要形成直觉，注意由上面的引理可知，
...
这可由 \cref{thm:sign-induction} 和群胚律轻松得到。
\end{proof}
...
\begin{proof}
  ...
  \narrowequation{\eqv{\sm{a:Y+Z}C(a)}{\sm{y:Y}E_Y(y)+\sm{z:Z}E_Z(z)},}
  因此
...
\begin{proof}
  ...
  \item 若 $x$ 是 $\base$，则按定义 $\mu(\base,\base)=\base$，故有
    \narrowequation{\refl\base:\mu(\base,\base)=\base.}
...
\begin{proof}[\cref{thm:freudenthal} 的证明]
  ...
  \begin{narrowmultline*}
    \code(\north,\refl{\north}) \jdeq \narrowbreak
    \trunc{2n}{\tsm{x:X} (\merid(x) \ct \opp{\merid(x_0)} = \refl{\north})}.
  \end{narrowmultline*}
  因而可取
  \begin{narrowmultline*}
    c(\north,\refl{\north}) \defeq \narrowbreak
    \tproj{2n}{(x_0,\mathsf{rinv}_{\merid(x_0)})},
  \end{narrowmultline*}
  其中 $\mathrm{rinv}_q$ 是任意 $q$ 的显然路径
  \narrowequation{q\ct\opp q = \refl{}.}
  现在可通过对 $p$ 做路径归纳得到
  \narrowequation{c:\prd{y:\susp X}{p:\north=y} \code(y,p),}
  但下文中我们还需要它的一个具体传送表达：
  \begin{narrowmultline*}
    c(y,p) \defeq \narrowbreak
    \transfib{\hat{\code}}{\pairpath(p,\mathsf{tid}_p)}{c(\north,\refl{\north})}
  \end{narrowmultline*}
...
（用 \cref{cha:category-theory} 的语言，这些将是“预群胚”。）
细节留给读者。
