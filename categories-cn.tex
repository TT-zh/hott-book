...
我们从一个预范畴 $X$ 出发。在应用到单排序一阶结构时，$X$ 将是 %\uset%
...
\begin{eg}\label{ct:sip-functor-cat}
  ...
  现在在 $B^{A_0}$ 上定义一种结构概念：其中 $P(F_0)$ 是操作 $F:\prd{a,a':A_0} \hom_A(a,a') \to \hom_B(F_0 a,F_0 a')$ 的类型，这些操作把 $F_0$ 扩展为函子（即保持复合与恒等）。
  ...
  $F'f = F'f \circ 1_{F_0 a} = 1_{F_0 a}\circ F f = F f$。
  施加函数外延性可得 $F = F'$。
...
\begin{proof}[第二种证明]
  ...
  注意这一步与 \cref{sec:compute-coprod,sec:compute-nat,cha:homotopy} 中的 encode-decode 方法非常相似。一次我们又是在刻画某个高阶归纳类型（这里是 $\widehat A_0$）的恒等类型：递归定义一族编码（这里是 $(x,y)\mapsto (x\cong y)$），并通过对 $\widehat A_0$ 及路径归纳定义编码与解码函数。
...
\begin{eg}\label{ct:rezk-fundgpd-trunc1}
  回忆 \cref{ct:fundgpd}：任意类型 $X$ 都有一个原群胚，其对象类型为 $X$，且 $\hom(x,y) \defeq \pizero{x=y}$。
...
\begin{ex}\label{ex:rezk-vankampen}
  ...
  \index{fundamental!pregroupoid}%
  使用 \cref{ct:fundgpd,ct:rezk-fundgpd-trunc1} 的基本（原）群胚与 \cref{sec:rezk} 的 Rezk 完备化，给出 van Kampen 定理（\cref{sec:van-kampen}）的另一种证明。
\end{ex}
