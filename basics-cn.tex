...
\cref{lem:opp} 的证明将构造该类型的一个元素，即对某个 $f$ 推导判断 $f:\prd{A:\UU}{x,y:A} (x= y)\to(y= x)$。
...
\begin{proof}[第一种证明]
  ...
  对每个 $p:(x= y)$ 成立。
  现在可把所需函数 $\opp{(\blank)}$ 定义为 $\lam{p} \indid{A}(D,d,x,y,p)$，即设 $\opp{p} \defeq \indid{A}(D,d,x,y,p)$。
...
\begin{proof}[第一种证明]
  ...
  \begin{equation*}
    d\defeq\lam{x} \refl{f(x)}:\prd{x:A} D(x,x,\refl{x})
  \end{equation*}
  现由路径归纳可得：对每个 $p:x= y$，有 $\apdfunc f(p):\trans p{f(x)}= f(y)$。
\end{proof}

\begin{proof}[第二种证明]
  由归纳法，只需考虑 $p$ 为 $\refl x$。
  但此时所需等式为 $\trans{(\refl{x})}{f(x)}= f(x)$，它按判定相等成立。
\end{proof}
...
因此，对任意 $x,y:A$、$p:x=y$ 与 $f:A\to B$，分别与 $\transconst Bp{f(x)}$ 及其逆拼接，可得到函数
\begin{align}
...
\]
而且这个映射是等价，因为
$\transfibf{C}(\alpha)$ 总是一个等价，其逆为
$\transfibf{C}{(\opp \alpha)}$，见 \cref{thm:transport-concat,thm:omg}。
...
这些结构使每个类型成为一个 1-groupoid（直到同伦）这一事实，在~\cite{hs:gpd-typethy} 中被用于给出类型论的首个“同伦式”语义。

把恒等类型解释为路径空间、把类型族解释为纤维化的真正同伦解释归功于 \cite{AW}，他们使用了 Quillen model categories 形式主义。一个在（严格）$\infty$-groupoids\index{.infinity-groupoid@$\infty$-groupoid} 中的解释也见论文 \cite{mw:thesis}。
关于在类型论中构造 $\infty$-groupoid 的\emph{全部}高阶运算与相容性，见~\cite{pll:wkom-type} 与~\cite{bg:type-wkom}。
...
受 groupoid 模型启发，Hofmann 与 Streicher~\cite{hs:gpd-typethy} 也提出了一个类似公理，称为“universe extensionality”。
