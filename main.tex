...
% Localization flag (enabled in hott-book-cn.tex).
\expandafter\ifx\csname ifOPTchinese\endcsname\relax
\expandafter\newif\csname ifOPTchinese\endcsname
\csname OPTchinesefalse\endcsname
\fi
...
% Chinese support for the XeLaTeX-based Chinese target.
\ifOPTchinese
...
% Default line spacing factor; individual top-level targets may override.
\expandafter\ifx\csname OPTlinespread\endcsname\relax
\def\OPTlinespread{1.05}
\fi

% Default paragraph indent; individual top-level targets may override.
\expandafter\ifx\csname OPTparindent\endcsname\relax
\def\OPTparindent{17pt}
\fi
...
\setlength{\parindent}{\OPTparindent}

\usepackage{ifpdf}

\usepackage{graphicx}
\DeclareGraphicsExtensions{.png}
% bmpsize-hack relies on pdfTeX internals and breaks under XeTeX.
...
\ifOPTchinese
\renewcommand{\chaptermark}[1]{\markboth{\textsc{第\thechapter 章\ #1}}{}}
\else
...
\ifOPTchinese
\titleformat{\part}[display]{\fontsize{\OPTpartfont}{\OPTpartfont}\fontseries{m}\fontshape{sc}\selectfont}{\hfil 第\ \Roman{part}\ 部}{\OPTpartskip}{\fontsize{\OPTparttitlefont}{\OPTparttitlefont}\fontseries{b}\fontshape{sc}\selectfont\hfil}
\else
...
\fi
\ifOPTchinese
\titleformat{\chapter}[display]{\fontsize{\OPTchapterfont}{\OPTchapterfont}\fontseries{m}\fontshape{it}\selectfont}{第\thechapter 章}{\OPTchapterskip}{\fontsize{\OPTchaptertitlefont}{\OPTchaptertitlefont}\fontseries{b}\fontshape{n}\selectfont}
\else
...
\ifOPTchinese
\include{front-cn} %%% Title page and copyright (Chinese)
\else
...
\ifOPTchinese
\include{equivalences-cn}
\else
\include{equivalences}
\fi

\ifOPTchinese
\include{induction-cn}
\else
\include{induction}
\fi
...
\ifOPTchinese
\renewcommand{\chaptermark}[1]{\markboth{\textsc{附录\thechapter. #1}}{}}
\else
