...
\end{itemize}
生成。
如果把类型看作高阶原群，那么这种更一般的“生成”概念就非常自然：
由于高阶原群是一个包含点、路径及更高阶路径的“多类对象”，我们理应允许在所有维度上给出“生成元”。
...
特别地，由于高阶原群自带路径与更高阶路径上的\emph{运算}，当这样的对象由若干构造子“生成”时，这些运算会产生一些并非直接来自构造子本身的路径。
...
从范畴论角度看，这与上文提到的现象本质相同：$\Sn^1$ 不仅包含 $\lloop$，还包含 $\lloop\ct\lloop$ 等等；差别只在于，对于\emph{高阶}原群，存在会提升维度的\emph{运算}。
...
不过在 \cref{sec:rezk} 中，我们会考虑类型按一个 $1$-原群取“商”，这是比集合商高一层的情形。）
...
（从范畴论观点看，$\sm{x:W} P(x)$ 是 $P$ 的“Grothendieck\index{Grothendieck construction} 构造”，而展平引理把其泛性质表达为一个“lax\index{lax colimit} 余极限”。由于同伦类型论中的类型（如 $W$）在范畴上对应 $\infty$-原群（因为所有路径可逆），在这里 lax 余极限与 pseudo 余极限是一致的。）
